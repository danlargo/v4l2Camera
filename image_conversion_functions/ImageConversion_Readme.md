...
- During the creation of the camControl project I wanted to make the image fetch and save functions as generic as possible and not lean on any other apis or libraries. This meant that I had to understand the various image formats that I would encounter. I was hoping that the Qt5 apis would provide the required functi0onkaity, unfortunately that did not seem to be the case. While many were supported (JPG and PNG) fotr example, some of the formats encountered in the UVC cameras had to be decoded directly. I have included some of the decoding functions I created to save other people the tie and frustration that I encountered trying to find these routines.

- **Image conversion flow** :
  * Motion-JPEG (fourCC code MJPG) : I used the Qt5 QImage conversion capabilities directly to read the image frame, and output as PNG
  * YUV 420 and YUV422 formats : The frames were converted to RGB888 (not RGB32 which would have included an alpha value), which were then converted into QImage format, that allowed it to be saved as PNG or JPG
  * H.264, H.265 : These frame types are not currently supported in camControl of tiny4KPanel.

- **Image Frame to Video conversion flow** :
  * The first pass at creating MP4 output files by saving the raw MJPG or H264/265 frames one by one to a MP4 file was only partially successful.
  * Without providing the correct frame rate and other header information the MP4 video file was not playable by some video apps.
  * ffplay was able to play the file, and if the file was run through ffmpeg and output to a new file, the frame rate and encoding information was correctled and the file was playable.

- **Video encoding Using OpenCV** :
  * At the end of the day I gave in and used the openCV api to provide the output encoding of the image frames into MP4 format.
  * MJPG frames were written directly to the output decoder (after specifying the file format, frame rate and resolution)
  * H264/H265 frames were written directly to the output decoder (after specifying the file format, frame rate and resolution)
  * YUV 422 and YUV420 frames were trans-coded into RGB888 frames and then written to the output decoder (after specifying the file format, frame rate and resolution)

- Supported fourCC image formats (fetch and convert ) in camControl (tiny4KPanel only supports Motion-JPEG)
...
- These functions are a result of extensive but not exhaustive searching. I ended up finding this information in bits and pieces across many different web sites. I did not keep track of any of the reference sites (sorry) although wikipedia and stack overflow were a great source, but sometimes conflicting source, of information.

- This is obviously not a comprehensive list of conversion functions, and the first place I would turn if I wanted to integrate well know iomage and vidoe processing APIs would be :
...
- Given RGB was 8 bits per pixel both 8 and 16 bit grey scale converted into the same format. The extra resolution in the 16-bit grey scale is lost when converting to RGB. 
